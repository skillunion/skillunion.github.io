...
Ext.define('SU.Component', {override:'Ext.Component', config:{formBind:false}});
...
Ext.define('SU.Button', {override:'Ext.Button', config:{toggleGroup:null}, updateToggleGroup:function(value, old) {
  var me = this;
  if (old) {
    me.un({pressedchange:me.onPressedChange, scope:me});
  }
  if (value) {
    me.on({pressedchange:me.onPressedChange, scope:me});
  }
}, onPressedChange:function(button, pressed) {
  var me = this, group = me.getToggleGroup(), CQ = Ext.ComponentQuery;
  if (!pressed) {
    return;
  }
  if (group) {
    CQ.query('button[toggleGroup\x3d' + group + '][pressed]').forEach(function(btn) {
      if (btn === button) {
        return;
      }
      btn.suspendEvents();
...
Ext.define('SU.field.Field', {override:'Ext.field.Field', requres:['Ext.Sheet'], config:{helperIconCls:'x-mi mi-help-circle-outline', helperTitle:undefined, helperText:undefined, helperConfig:{xtype:'sheet', cls:'x-field-helper-sheet'}}, preventMark:false, defaultHelperConfig:{showAnimation:false, hideAnimation:false, centered:false}, listeners:{click:'onHelperClick', element:'labelHelperElement', scope:'this'}, destroy:function() {
  var me = this;
  Ext.destroy(me.helperSheet);
  me.callParent(arguments);
}, getTemplate:function() {
  var template = this.callParent(arguments), helperHidden = !this.getHelperText();
  template[0].children.push({reference:'labelHelperElement', cls:Ext.baseCSSPrefix + 'label-helper-el', tag:'span', html:'\x26nbsp;', hidden:helperHidden});
  return template;
...
}, privates:{getHelperSheet:function() {
  var me = this, config = Ext.applyIf(me.getHelperConfig() || {}, me.defaultHelperConfig || {});
  return me.helperSheet || (me.helperSheet = Ext.factory(Ext.applyIf(config, {title:me.getHelperTitle(), html:me.getHelperText()}), Ext.Sheet));
}}});
Ext.define('SU.field.Text', {override:'Ext.field.Text', config:{maskRe:undefined, maskRawValue:false}, initialize:function() {
  var me = this;
  me.callParent(arguments);
  if (me.getMaskRe()) {
  ...
  var charCode = String.fromCharCode(e.getCharCode()), raw = this.getValue() || '', value = this.getMaskRawValue() ? raw + charCode : charCode;
  if (!this.getMaskRe().test(value)) {
    e.stopEvent();
  }
}});
Ext.define('SU.dataview.selection.Model', {override:'Ext.dataview.selection.Model', selectWithEventMulti:function(record, e, isSelected) {
  var me = this, shift = e.shiftKey, ctrl = e.ctrlKey, key = e.getKey(), start = shift ? me.selectionStart : null;
  if (shift && start) {
    me.selectRange(start, record, ctrl);
  } else {
    if (~[33, 34, 35, 36, 37, 38, 39, 40].indexOf(key) && !shift && !ctrl) {
      return;
    }
    isSelected ? me.deselect(record) : me.select(record, true);
  }
}});
Ext.define('SU.dataview.Abstract', {override:'Ext.dataview.Abstract', onInnerFocusEnter:function(e) {
...
Ext.define('SU.dataview.plugin.ListPaging', {override:'Ext.dataview.plugin.ListPaging', privates:{doEnsureBufferZone:function() {
...
Ext.define('SU.field.File', {override:'Ext.field.File'}, function(Cls) {
  var proxyConfig = Cls.prototype.proxyConfigs;
  Ext.mixin.ConfigProxy.processClass(Cls, proxyConfig);
});
Ext.define('SU.field.Search', {override:'Ext.field.Search', alternateClassName:['Ext.form.field.Search', 'Ext.form.SearchField'], config:{reuse:true}, initialize:function() {
  var me = this, search = me.getTriggers().search;
  me.callParent();
...
Ext.define('SU.grid.plugin.CellEditing', {override:'Ext.grid.plugin.CellEditing', getEditor:function(location) {
  if (!location.column) {
    return;
  }
...
Ext.define('SU.ErrorDialog', {extend:'Ext.Dialog', alias:'widget.errordialog', alternateClassName:['Ext.ErrorDialog'], width:'90%', minWidth:320, maxWidth:600, closable:true, cls:'x-error-dialog', textOk:'Ok', textAdvansed:'Advanced', textErrorTitle:'Error message', textErrorNumber:'Error {0} of {1}', keyMap:{ESC:'onEscape', ENTER:'onEscape', scope:'this'}, initialize:function() {
  var me = this;
  ...
  var me = this;
  me.navigate = me.navigate || me.add({xtype:'toolbar', docked:'bottom', ui:'upper'});
  me.errorNumber = me.errorNumber || me.navigate.add({xtype:'label', flex:1, ui:'light'});
  me.btnPrev = me.btnPrev || me.navigate.add({width:50, text:'\x3c\x3c', cls:'x-error-btn-prev', focusable:false, handler:'onPrev', scope:me});
  me.btnNext = me.btnNext || me.navigate.add({width:50, text:'\x3e\x3e', cls:'x-error-btn-next', focusable:false, handler:'onNext', scope:me});
}, initMessageInfo:function() {
  var me = this, container = me.add({xtype:'container', layout:{type:'hbox', align:'stretch', pack:'end'}}), buttons;
  me.icon = container.add({xtype:'component', itemId:'erroricon', cls:'x-error-icon'});
  me.message = container.add({xtype:'component', itemId:'errormessage', height:125, flex:1, cls:'x-error-message', scrollable:true});
  buttons = container.add({xtype:'container', width:115, layout:{align:'stretch', type:'vbox'}});
  me.btnOk = buttons.add({xtype:'button', margin:5, ui:'action', text:me.textOk, handler:'onOk', scope:me});
  me.btnAdvansed = buttons.add({xtype:'button', docked:'bottom', margin:5, ui:'action', text:me.textAdvansed, focusable:false, enableToggle:true, toggleHandler:'onAdvansed', scope:me});
}, initDescription:function() {
  var me = this;
  me.description = me.description || me.add({xtype:'container', itemId:'description', scrollable:true, border:1, cls:'x-error-description'});
  ...
  config = config || {};
  var me = this, index = config.count ? config.current || 0 : -1, prevEnable = index > 0 && config.count > 1, nextEnable = index < config.count - 1 && config.count > 1, visible = config.count > 1;
  ...
  var me = this, description = exception.description || '', msg = exception.message || exception.msg || exception.toString();
  ...
  if (exception.stack && me.statics().APPEND_STACK && (me.statics().APPEND_STACK_EMPTY_DESCRIPTION || !!description)) {
    description += '\n\n';
  ...
  var me = this, errorDescription = me.down('#errordescription'), errorAdvansed = me.down('#erroradvansed'), visibled = errorAdvansed.pressed && errorAdvansed.isVisible();
...
}, statics:{APPEND_STACK:true, APPEND_STACK_EMPTY_DESCRIPTION:false}});
Ext.define('SU.LangSelect', {extend:'Ext.field.Select', alias:'widget.languagefield', alternateClassName:['Ext.field.LangSelect', 'Ext.form.LangSelect'], config:{valueField:'id', displayField:'label', usePicker:false, autoSelect:false}, initialize:function() {
  var me = this;
  me.callParent();
  me.setStore(Ext.Language.getLocales());
  me.setValue(Ext.Language.getPersistedLocale(true));
  Ext.Language.on({changelocale:me.doLocaleChange, failurelocale:me.doLocaleFailure, scope:me});
}, destroy:function() {
  var me = this;
  Ext.Language.un({changelocale:me.doLocaleChange, failurelocale:me.doLocaleFailure, scope:me});
  me.callParent();
...
Ext.define('SU.PulseButton', {extend:'Ext.Component', alias:'widget.pulsebutton', floated:true, baseCls:'x-pulsebutton', cachedConfig:{eventHandlers:{click:'onClick'}}, config:{iconCls:null, handler:null, scope:null}, height:32, width:32, preventDefaultAction:true, element:{tag:'a', reference:'element'}, template:[{reference:'circleElement', style:'transform-origin: center;', cls:'x-pulse-circle'}, {reference:'fillElement', style:'transform-origin: center;', cls:'x-pulse-circle-fill'}, {reference:'innerElement', 
style:'transform-origin: center;', cls:'x-pulse-icon-circle', onclick:'return Ext.doEv(this, event);', children:[{reference:'iconElement', style:'transform-origin: center;', cls:'x-pulse-icon-circleblock'}]}], updateIconCls:function(iconCls, oldIconCls) {
  ...
  var me = this, element = me.iconElement, circle = me.circleElement;
...
Ext.define('SU.field.Captcha', {extend:'Ext.field.Field', alternateClassName:['Ext.field.Captcha', 'Ext.form.field.Captcha'], xtype:'captchafield', config:{siteKey:undefined, theme:'dark'}, isField:true, classCls:Ext.baseCSSPrefix + 'capchafield', capchaCls:'g-recaptcha', isCaptchaValid:false, initialize:function() {
  var me = this, po, s;
  me.callParent(arguments);
  ...
  return [{reference:'captchaElement', cls:this.capchaCls}];
  ...
  var me = this;
  grecaptcha.render(me.captchaElement.id, {'callback':function(response) {
...
Ext.define('SU.field.Tag', {extend:'Ext.field.Picker', alternateClassName:['Ext.field.Tag', 'Ext.form.field.Tag'], xtype:'tagfield', requires:['Ext.picker.Picker'], config:{store:null, displayField:'text', valueField:'id', queryMode:'local', queryField:undefined, floatedPicker:{xtype:'list', selectable:'multi'}, edgePicker:{xtype:'list', selectable:'multi'}}, isField:true, isInputField:false, isSelectField:true, classCls:Ext.baseCSSPrefix + 'tagfield', picker:'floated', listeners:{collapse:function() {
  var me = this, store = me.getStore(), queryField = me.getQueryField() || me.getDisplayField(), inputEl = me.inputElement.dom;
  inputEl.value = '';
  store.removeFilter(queryField);
}, keyup:{fn:function(field, e) {
  var me = this, v = me.getInputValue(), store = me.getStore(), queryMode = me.getQueryMode(), queryField = me.getQueryField() || me.getDisplayField();
  ...
  if (v && v.length) {
    store.addFilter({property:queryField, operator:'like', value:v});
  } else {
    store.removeFilter(queryField);
  }
}, buffer:200}}, applyDisplayField:function(displayField) {
  ...
  var me = this;
  return Ext.merge({ownerCmp:me, store:me.getStore(), itemTpl:me.itemTpl ? me.itemTpl : '{' + me.getDisplayField() + '}', listeners:{select:me.onSelect, deselect:me.onDeselect, scope:me}}, me.getFloatedPicker());
}, createEdgePicker:function() {
  var me = this;
  return Ext.merge({ownerCmp:me, store:me.getStore(), itemTpl:me.itemTpl ? me.itemTpl : '{' + me.getDisplayField() + '}', listeners:{select:me.onSelect, deselect:me.onDeselect, scope:me}}, me.getEdgePicker());
}, getValue:function() {
  var me = this, selected = me.getSelected(), keys = Object.keys(selected), values = [];
  ...
  var me = this, selection = [], queryMode = me.getQueryMode(), store = me.getStore(), promises = [];
  if (!store) {
    me.on({updatestore:function(store, oldStore) {
      if (store && !oldStore) {
        me.setValue(value);
      }
    }, single:true});
  ...
  this.callParent();
}, privates:{getSelected:function() {
  return this._selected || (this._selected = {});
}, onSelect:function(view, records) {
  records = Ext.Array.from(records);
  var me = this, field = me.getValueField(), selected = me.getSelected(), i = 0, len = records.length;
  while (i < len) {
  ...
  var me = this, i = 0, len = records.length;
  while (i < len) {
  ...
  var me = this, id = me.id + '-tag-' + tag.get(me.getValueField()), el = document.createElement('span');
  el.id = id;
  el.className = Ext.baseCSSPrefix + 'tag';
  el.innerHTML = tag.get(me.getDisplayField()) + ' \x3cspan class\x3d"x-tag-tool x-fa fa-close" aria-hidden\x3d"true"\x3e\x26nbsp;\x3c/span\x3e';
  el.setAttribute('tagFieldSpan', true);
  el.querySelector('span').addEventListener('click', Ext.bind(function() {
    this.getPicker().getSelectable().deselect([tag]);
    this.getPicker().onItemDeselect([tag]);
  }, me));
  me.beforeInputElement.append(el);
  me.beforeInputElement.setStyle({marginRight:'10px'});
  ...
  var items = this.beforeInputElement.query('span[tagFieldSpan\x3dtrue]'), i = items.length, el;
  while (i--) {
    el = this.beforeInputElement.down('#' + this.id + '-tag-' + items[i].get(this.getValueField()));
    el && el.destroy();
  }
}, syncLabelPlaceholder:function(animate) {
  var me = this, selected = me.getSelected(), inside;
...
Ext.define('SU.field.plugin.HighlightArea', {extend:'Ext.plugin.Abstract', alias:'plugin.highlightarea', config:{field:null, boundaries:null, scope:null}, init:function(field) {
  var me = this, container;
  me.setField(field);
  me.input = field.inputElement;
  field.inputElement.addCls('x-input x-content');
  var container = Ext.dom.Helper.insertAfter(me.input, {tag:'div', cls:'x-highlight-container', children:[{tag:'div', cls:'x-backdrop', children:[{tag:'div', cls:'x-highlights'}]}]});
...
Ext.define('SU.grid.TextAreaCellEditor', {extend:'Ext.grid.CellEditor', alias:'widget.textareacelleditor', field:{xtype:'textareafield'}, onSpecialKey:function(field, event) {
  var me = this;
...
Ext.define('SU.panel.Accordion', function(Accordion) {
  if (Ext.versions.extjs.gtEq('7.0')) {
    return {extend:'Ext.panel.Accordion'};
  }
  return {extend:'Ext.Panel', xtype:'accordion', mixins:['Ext.mixin.Bufferable'], requires:['Ext.layout.VBox', 'Ext.panel.Collapser'], config:{collapseByDefault:true, defaultPanelUI:'accordion', headerPanelClickable:true, animationPanel:false}, layout:{type:'vbox'}, bufferableMethods:{syncState:'asap'}, accordionCls:Ext.baseCSSPrefix + 'layout-accordion', accordionSelector:'\x3e [isPanel][collapsible][isInner]', initialize:function() {
    var me = this;
    me.callParent(arguments);
    me.on({scope:me, delegate:me.accordionSelector, priority:-1000, beforecollapse:'onPanelBeforeCollapse', beforeexpand:'onPanelBeforeExpand', beforehiddenchange:'onBeforePanelHiddenChange', collapse:'onPanelCollapse', expand:'onPanelExpand', hiddenchange:'onPanelHiddenChange'});
    me.getRenderTarget().addCls(me.accordionCls);
    me.syncStateNow();
  }, onItemAdd:function(item, index) {
    var me = this, initialItemConfig = item.initialConfig, startCollapsed = initialItemConfig.collapsed, animationPanel = me.getAnimationPanel(), collapsible;
    if (item.isPanel && item.isInnerItem()) {
      ...
      if (collapsible !== false) {
        ...
        if (collapsible) {
          ...
          item.setCollapsible({collapsed:Ext.isDefined(startCollapsed) ? startCollapsed : true, animation:animationPanel, dynamic:true, useDrawer:false});
        }
      }
    }
    me.callParent([item, index]);
  }, onItemRemove:function(item, index, destroying) {
    var me = this, collapsible;
    ...
    var me = this, panels = me.query(me.accordionSelector);
    panels.forEach(function(item) {
      me.bindHeaderEvent(item, value);
    });
  }, privates:{bindHeaderEvent:function(panel, set) {
    var me = this, header = panel.getHeader();
    fn = set ? 'on' : 'un';
    header[fn].call(header, {scope:me, click:'onPanelHeaderClick', element:'element'});
  }, getAccordionPanels:function() {
    var me = this, items = me.query(me.accordionSelector), expanded = [], n = items.length, i, item;
    for (i = 0; i < n; ++i) {
      item = items[i];
      if (item.getHidden()) {
        items.splice(i, 1);
    ...
    var me = this, panels = me.query(me.accordionSelector), expanded = me.query(me.accordionSelector + '[collapsed\x3dfalse]'), item;
