...
Ext.define('SU.native.overrides.fix.EventPublisherDom', {override:'Ext.event.publisher.Dom', doDirectEvent:function(e, capture) {
...
Ext.define('SU.native.overrides.app.Application', {override:'Ext.app.Application', init:function() {
  var me = this;
  if (window.cordova) {
    var mayBeRady = function() {
      if (nativeRady && extRady) {
        profilesReady.call(me);
      }
    };
    var profilesReady = me.onProfilesReady, nativeRady = false, extRady = false;
...
Ext.define('SU.native.overrides.MessageBox', {override:'Ext.MessageBox', nativeDialog:false, show:function(msgBoxOptions) {
  if (!navigator.notification || !this.nativeDialog) {
    return this.callParent(arguments);
  }
  var me = this, method, buttons;
  ...
  navigator.notification[method].call(me, msgBoxOptions.message || '', function(results) {
    results = results || {};
    var buttonIndex = (typeof results === 'number' ? results : results.buttonIndex) || 1, value = results.input1;
    if (msgBoxOptions.fn) {
      Ext.callback(msgBoxOptions.fn, msgBoxOptions.scope, [buttons[--buttonIndex].itemId, value]);
    }
  }, msgBoxOptions.title || 'Alert', buttons.map(function(btn) {
...
Ext.define('SU.native.overrides.Toast', {override:'Ext.Toast'}, function() {
...
Ext.define('SU.native.Network', {alternateClassName:['SU.Network'], singleton:true, requires:['Ext.Toast'], config:{signalling:true}, UNKNOWN:'unknown', ETHERNET:'enternet', WIFI:'wifi', CELL_2G:'2g', CELL_3G:'3g', CELL_4G:'4g', CELL:'cellular', NONE:'none', connectionType:function() {
  ...
  if (this.getSignalling()) {
    Ext.toast({message:SU.Network.connectionTypeName(), styling:{backgroundColor:this.connectionType() === this.NONE ? '#FF0000' : '#00FF00'}});
  }
...
Ext.define('SU.native.tab.SlidePanel', {extend:'Ext.carousel.Carousel', alias:'widget.slidetabpanel', requires:['Ext.tab.Bar', 'Ext.tab.Tab'], config:{tabBar:true, tabBarPosition:'top', indicator:false}, defaults:{allowHeader:false}, initialize:function() {
  var me = this;
  me.callParent();
  ...
  if (newActiveItem) {
    ...
    if (oldActiveItem) {
      oldActiveItem.removeCls('x-carousel-item-active');
    }
    if (newActiveItem) {
      newActiveItem.addCls('x-carousel-item-active');
    }
    if (newIndex !== -1) {
